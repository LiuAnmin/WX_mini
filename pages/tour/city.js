...
Page({
  ...
  data: {
    ...
    count: 0,
    multiIndex: [0, 0],
    multiArray: [provinceData, ['北京']],
    objectMultiArray: cityData,
    MSG: cur_MSG,
    noProvince: 0
  },
  onLoad: function () {
  ...
  bindMultiPickerChange: function (e) {
    regidx = cur_reglist[e.detail.value[1]] - 2
    //console.log(regidx)
    cur_sel = cityData[regidx].ageid
    //console.log(cur_sel)
    cityData[regidx].ageid = 1 - cur_sel
    if (cur_sel == 0) {
      cur_MSG = "已选"
    } else {
      cur_MSG = "已删"
    ...
    this.setData({
      ...
      "MSG": cur_MSG,
      "count": this.data.count + (1 - 2 * cur_sel),
    })
  },
  bindMultiPickerColumnChange: function (e) {
    if (e.detail.column == 0) {
      list = []
      cur_province = cityData[e.detail.value].regid
      cur_reglist = []
      for (var i = 0; i < cityData.length; i++) {
        if (cityData[i].parid == cur_province) {
          list.push(cityData[i].regname)
          cur_reglist.push(cityData[i].regid)
        }
      }
      regidx = cur_reglist[0] - 2  
      this.setData({
        "multiArray[1]": list,
      ...
      regidx = cur_reglist[e.detail.value] - 2
      this.setData({
  ...
  cal_cnt: function (e) {
    ...
    for (var i = 0; i < cityData.length; i++) {
      if (cityData[i].ageid == 1) {
        cnt_perP[cityData[i].parid - 2] = cnt_perP[cityData[i].parid - 2] + 1
      }
    }
    console.log(`每省计数：${cnt_perP}`)
    cnt_Province = 0
    for (var i = 0; i < provinceData.length; i++) {
      if (cnt_perP[i] > 0)
        cnt_Province++
  ...
  onShowmap: function (e) {
    ...
    for (var i = 0; i < provinceData.length; i++) {
      if (cnt_perP[i] > 0) {
        mapdata.push({ name: provinceData[i], value: cnt_perP[i] * 10 / total_perP[i] })
      }
    }
    console.log(`地图数据：`)
    console.log(mapdata)
  ...
  getData: function () {
    myoption.series[0].data = mapdata
